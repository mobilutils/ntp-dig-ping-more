...
Une **commande pseudo** est un mot-clé intégré reconnu par la fonctionnalité Actions en Lots. Contrairement aux exécutables shell (par ex. `ping` ou `dig` qui invoquent des binaires système via `Runtime.exec`), les commandes pseudo déclenchent du code Kotlin personnalisé directement dans `BulkActionsRepository`. Il existe 9 commandes pseudo :
